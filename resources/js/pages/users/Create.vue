                ...
                <v-form ref="form" @submit.prevent="submitForm">
                    ...
                    <div class="d-flex justify-end ga-2">
                        ...
                        <v-btn color="primary" type="submit" :loading="loading">
                            <v-icon start>mdi-content-save</v-icon>Create User
                        </v-btn>
                    </div>
                </v-form>
            </v-card-text>
...
const submitForm = async () => {
    ...
    try {
        ...
        snackbar.message = 'User created successfully'; snackbar.color = 'success'; snackbar.show = true;
        setTimeout(() => router.push({ name: 'users.index' }), 1000);
    } catch (error) {
        snackbar.message = error.response?.data?.message || 'Failed to create user';
