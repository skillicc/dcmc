            ...
            <v-card-text>
                <v-form ref="form" @submit.prevent="submitForm">
                    <v-row>
                        ...
                        <v-col cols="12" md="6">
                            <v-text-field v-model="formData.duration" label="Duration (e.g., 2 hours)" prepend-inner-icon="mdi-clock" />
                        </v-col>
                        <v-col cols="12" md="6">
                        ...
                        <v-col cols="12">
                            <v-textarea v-model="formData.normal_range" label="Normal Range / Reference Values" rows="3" prepend-inner-icon="mdi-chart-line" />
                        </v-col>
                    </v-row>
                    ...
                    <div class="d-flex justify-end ga-2">
                        ...
                        <v-btn color="primary" type="submit" :loading="loading">
                            <v-icon start>mdi-content-save</v-icon>Save Test
                        </v-btn>
                    </div>
                </v-form>
            </v-card-text>
...
const submitForm = async () => {
    ...
    try {
        ...
        snackbar.message = 'Test created successfully'; snackbar.color = 'success'; snackbar.show = true;
        setTimeout(() => router.push({ name: 'tests.index' }), 1000);
    } catch (error) {
        snackbar.message = error.response?.data?.message || 'Failed to create test';
